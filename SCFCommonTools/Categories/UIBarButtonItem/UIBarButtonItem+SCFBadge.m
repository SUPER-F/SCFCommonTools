...
NSString const *scf_UIBarButtonItem_badgeKey = @"scf_UIBarButtonItem_badgeKey";

NSString const *scf_UIBarButtonItem_badgeBGColorKey = @"scf_UIBarButtonItem_badgeBGColorKey";
NSString const *scf_UIBarButtonItem_badgeTextColorKey = @"scf_UIBarButtonItem_badgeTextColorKey";
NSString const *scf_UIBarButtonItem_badgeFontKey = @"scf_UIBarButtonItem_badgeFontKey";
NSString const *scf_UIBarButtonItem_badgePaddingKey = @"scf_UIBarButtonItem_badgePaddingKey";
NSString const *scf_UIBarButtonItem_badgeMinSizeKey = @"scf_UIBarButtonItem_badgeMinSizeKey";
NSString const *scf_UIBarButtonItem_badgeOriginXKey = @"scf_UIBarButtonItem_badgeOriginXKey";
NSString const *scf_UIBarButtonItem_badgeOriginYKey = @"scf_UIBarButtonItem_badgeOriginYKey";
NSString const *scf_UIBarButtonItem_shouldHideBadgeAtZeroKey = @"scf_UIBarButtonItem_shouldHideBadgeAtZeroKey";
NSString const *scf_UIBarButtonItem_shouldAnimateBadgeKey = @"scf_UIBarButtonItem_shouldAnimateBadgeKey";
NSString const *scf_UIBarButtonItem_badgeValueKey = @"scf_UIBarButtonItem_badgeValueKey";
...
- (void)scf_badgeInit {
    ...
    if (self.customView) {
        ...
        defaultOriginX = superview.frame.size.width - self.scf_badge.frame.size.width / 2.0;
        // Avoids badge to be clipped when animating its scale
        superview.clipsToBounds = NO;
    }
    else if ([self respondsToSelector:@selector(view)] && [(id)self view]) {
        superview = [(id)self view];
        defaultOriginX = superview.frame.size.width - self.scf_badge.frame.size.width;
    }
    [superview addSubview:self.scf_badge];
    ...
    self.scf_badgeOriginX = defaultOriginX;
...
- (UILabel *)scf_badge {
    UILabel *lb = objc_getAssociatedObject(self, &scf_UIBarButtonItem_badgeKey);
    if (!lb) {
...
- (void)setScf_badge:(UILabel *)scf_badge {
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_badgeKey, scf_badge, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

#pragma mark scf_badgeValue
- (NSString *)scf_badgeValue {
    return objc_getAssociatedObject(self, &scf_UIBarButtonItem_badgeValueKey);
}

- (void)setScf_badgeValue:(NSString *)scf_badgeValue {
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_badgeValueKey, scf_badgeValue, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    // 处理改变value
    [self scf_updateBadgeValueAnimated:YES];
...
- (UIColor *)scf_badgeBGColor {
    return objc_getAssociatedObject(self, &scf_UIBarButtonItem_badgeBGColorKey);
}

- (void)setScf_badgeBGColor:(UIColor *)scf_badgeBGColor {
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_badgeBGColorKey, scf_badgeBGColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.scf_badge) {
        [self scf_refreshBadge];
    }
}
...
- (UIColor *)scf_badgeTextColor {
    return objc_getAssociatedObject(self, &scf_UIBarButtonItem_badgeTextColorKey);
}

- (void)setScf_badgeTextColor:(UIColor *)scf_badgeTextColor {
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_badgeTextColorKey, scf_badgeTextColor, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.scf_badge) {
        [self scf_refreshBadge];
    }
}
...
-  (UIFont *)scf_badgeFont {
    return objc_getAssociatedObject(self, &scf_UIBarButtonItem_badgeFontKey);
}

- (void)setScf_badgeFont:(UIFont *)scf_badgeFont {
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_badgeFontKey, scf_badgeFont, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.scf_badge) {
        [self scf_refreshBadge];
    }
}
...
- (CGFloat)scf_badgePadding {
    NSNumber *number = objc_getAssociatedObject(self, &scf_UIBarButtonItem_badgePaddingKey);
    return number.floatValue;
}

- (void)setScf_badgePadding:(CGFloat)scf_badgePadding {
    NSNumber *number = [NSNumber numberWithDouble:scf_badgePadding];
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_badgePaddingKey, number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.scf_badge) {
        [self scf_updateBadgeFrame];
    }
}
...
- (CGFloat)scf_badgeMinSize {
    NSNumber *number = objc_getAssociatedObject(self, &scf_UIBarButtonItem_badgeMinSizeKey);
    return number.floatValue;
}

- (void)setScf_badgeMinSize:(CGFloat)scf_badgeMinSize {
    NSNumber *number = [NSNumber numberWithDouble:scf_badgeMinSize];
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_badgeMinSizeKey, number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.scf_badge) {
        [self scf_updateBadgeFrame];
    }
}
...
- (CGFloat)scf_badgeOriginX {
    NSNumber *number = objc_getAssociatedObject(self, &scf_UIBarButtonItem_badgeOriginXKey);
    return number.floatValue;
}

- (void)setScf_badgeOriginX:(CGFloat)scf_badgeOriginX {
    NSNumber *number = [NSNumber numberWithDouble:scf_badgeOriginX];
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_badgeOriginXKey, number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.scf_badge) {
        [self scf_updateBadgeFrame];
    }
}
...
- (CGFloat)scf_badgeOriginY {
    NSNumber *number = objc_getAssociatedObject(self, &scf_UIBarButtonItem_badgeOriginYKey);
    return number.floatValue;
}

- (void)setScf_badgeOriginY:(CGFloat)scf_badgeOriginY {
    NSNumber *number = [NSNumber numberWithDouble:scf_badgeOriginY];
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_badgeOriginYKey, number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.scf_badge) {
        [self scf_updateBadgeFrame];
    }
}
...
- (BOOL)scf_shouldHideBadgeAtZero {
    NSNumber *number = objc_getAssociatedObject(self, &scf_UIBarButtonItem_shouldHideBadgeAtZeroKey);
...
- (void)setScf_shouldHideBadgeAtZero:(BOOL)scf_shouldHideBadgeAtZero {
    ...
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_shouldHideBadgeAtZeroKey, number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.scf_badge) {
        [self scf_refreshBadge];
    }
}
...
- (BOOL)scf_shouldAnimateBadge {
    NSNumber *number = objc_getAssociatedObject(self, &scf_UIBarButtonItem_shouldAnimateBadgeKey);
    return number.boolValue;
}

- (void)setScf_shouldAnimateBadge:(BOOL)scf_shouldAnimateBadge {
    NSNumber *number = [NSNumber numberWithBool:scf_shouldAnimateBadge];
    objc_setAssociatedObject(self, &scf_UIBarButtonItem_shouldAnimateBadgeKey, number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    if (self.scf_badge) {
        [self scf_refreshBadge];
    }
}
