...
#import "NSDate+SCFInternetDateTime.h"
...
@implementation NSDate (SCFInternetDateTime)
...
+ (NSDate *)dateFromInternetDateTimeString:(NSString *)dateString formatHint:(SCFDateFormatHint)hint {
    // Keep dateString around a while (for thread-safety)
    NSDate *date = nil;
    if (dateString) {
        if (hint != SCFDateFormatHintRFC3339) {
            // Try RFC822 first
